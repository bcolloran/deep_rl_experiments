...
from ddqn_udacity.ddqn_agent import Agent
...
dist_choices, yaw_choices = np.meshgrid([-1, 0, 1], [-1, 0, 1])
action_choices = np.vstack((np.ravel(dist_choices), np.ravel(yaw_choices)))
...
def runner(
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        for t in range(max_t):
            ...
            mapped_action = action_choices[:, action]
...
def trainer():
    # env = GameEnv(N_Agents=5, enemies_stationary=True)
    env = GameEnv(N_agents=12, enemy_type="stationary")
    ...
    agent = Agent(
        state_size=env.getStateDimension(), action_size=9, seed=123, device=device
    )
